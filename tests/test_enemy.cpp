...
TEST_CASE("Enemy takes damage correctly", "[Enemy]") {
    Enemy enemy("Test Goblin", "A weak goblin.", 1, 50, 5, 10, 2, 1, EnemyType::GOBLINOID,
                EnemyRarity::COMMON);
...
TEST_CASE("Enemy takes spell damage correctly", "[Enemy]") {
    Enemy enemy("Test Skeleton", "A bony skeleton.", 1, 40, 4, 8, 1, 3, EnemyType::UNDEAD,
                EnemyRarity::COMMON);
...
TEST_CASE("Enemy heals correctly", "[Enemy]") {
    Enemy enemy("Test Orc", "A strong orc.", 1, 60, 8, 14, 3, 1, EnemyType::HUMANOID,
...
TEST_CASE("Enemy attack returns value within range", "[Enemy]") {
    Enemy enemy("Test Dragon", "A fierce dragon.", 1, 200, 20, 40, 10, 5, EnemyType::DRAGON,
                EnemyRarity::EPIC);
    for (int i = 0; i < 100; ++i) {
...
TEST_CASE("Enemy isAlive works correctly", "[Enemy]") {
    Enemy enemy("Test Demon", "A terrifying demon.", 1, 80, 15, 25, 5, 10, EnemyType::DEMON,
                EnemyRarity::RARE);
...
TEST_CASE("Enemy getters return correct values", "[Enemy]") {
    Enemy enemy("Test Elemental", "A mystical elemental.", 1, 100, 10, 20, 4, 8,
